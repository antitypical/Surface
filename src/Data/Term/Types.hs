...
expected (Unification out) = Term Set.empty (const . const $ Left "Unification does not preserve typecheckers.\n") (expected <$> out)

actual :: Functor f => Unification f -> Term f
actual (Conflict _ actual) = actual
actual (Unification out) = Term Set.empty (const . const $ Left "Unification does not preserve typecheckers.\n") (actual <$> out)
...
unified (Unification out) = do
  ...
  return $ Term Set.empty (const . const $ Left "Unification does not preserve typecheckers.\n") out
